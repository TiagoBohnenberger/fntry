...
public interface Step<T> {
    ...
    /**
     * Receives a {@link ThrowingSupplier} and delegates to {@link Step#orElse(Object) orElse(T)}
     *
     * @param other the supplier function
     * @return the fallback object's type
     */
    default T orElseGet(ThrowingSupplier<T, ? extends Throwable> other) {
        try {
    ...
    default void orElse(Consumer<T> fallbackConsumer) {
        Result<T> result = getResult();
        if (result.isFailure()) {
            fallbackConsumer.accept(result.get());
        }
    }
}